...
\begin{abstract}
  Kepler\,1627 is an active Sun-like star previously found to host a
  $3.56\pm 0.04\,R_\oplus$ mini-Neptune on a 7.2\,day orbit.  The star was
  selected for observation by the Kepler satellite because it is
  nearby and Sun-like.  Here, combining data from the Gaia satellite
  with rotation periods from TESS, we show that Kepler\,1627 is a
  member of the underappreciated \cn.  Based on isochronal and
  gyrochronal evidence, the cluster is $30\pm
  XX$\,Myr old.  Based on archival NIRC2 imaging, we also highlight the existence of 
  a previously unknown stellar companion. Our analysis advances Kepler\,1627Ab as the youngest planet with a
  precise age observed by the main Kepler mission.  Newly
  derived ages from Gaia offer the opportunity to significantly expand
  the census of age-dated planets -- we offer a literature compilation
  of young stars to support this effort.  The properties of \pn\
  may also help clarify how the orbits and atmospheres of the
  mini-Neptune planets evolve.
\end{abstract}
...
\begin{figure*}[t]
	...
	\vspace{-0.7cm}
	\caption{
		{\bf 100 day window of the \sn\ light curve.}  
		The full Kepler dataset spans 1{,}437 days (3.9 years), sampled at
		30 minute cadence.  The {\it top panel} shows the
...
\begin{figure*}[t]
	...
	\caption{
		{\bf Phase-folded transit of Kepler 1627b, with stellar
			variability removed.}  
		The 1-$\sigma$ model uncertainties and the mean model are shown
		as the faint purple band, and the dark purple line.
		Gray points are individual \texttt{PDCSAP} flux measurements;
		black points are binned to 20 minutes, with a representative error
		bar shown.
...
notable discoveries.  The first, to be discussed in an upcoming analysis
...
While we have yet to perform an in-depth search of the {\bf XXX} light
curves Kepler acquired for the \cn, there is already a known planet
that has been detected in the cluster: Kepler\,1627b.  A portion of
the light curve is shown in Figure~\ref{fig:lightcurve}, and the
phase-folded transit is shown in Figure~\ref{fig:phasefold}.  The
dominant signal in the \texttt{PDCSAP} photometry is a quasi-period
starspot signal with a peak-to-peak amplitude that varies between
roughly 2\% and 8\%, depending on the degree of asymmetry in spot
coverage between the stellar hemispheres.  The starspot signal repeats
roughly every 2.606\,days, though this value varies by a few percent
depending on which portion of the light curve is analyzed {\bf (FIXME:
	do!)}.  The details of how we model this signal to derive the
parameters of the planet are discussed in Section~\ref{app:gptransit}.
The results, given in Table~\ref{tab:posterior}, are consistent with a
mini-Neptune sized planet ($3.56\pm 0.04\,R_\oplus$) on a circular
7.2\,day orbit, around a G8V star ($0.91 \pm 0.05 R_\odot$) {\bf
	fixme: age}. 
...
background eclipsing binary.  \citet{morton_false_2016} reported the latter
scenario to be the only possible case, with a (model-depending)
...
positives \citep{shporer_three_2017}.  We are not particularly
concerned in this case however, since these cases almost always
originate when attempting to separate the smallest stars from
identically-sized gas giant planets.  With a size of $\approx
3.6\,R_\oplus$, Kepler\,1627b is well below the regime of concern.

Nonetheless, our analysis of archival Keck-NIRC2 Kp-band (2.12\,$\mu
$m) imaging did reveal the existence of a neighboring star, unresolved
in the Gaia source catalog {\bf FIXME: uncertainties}.  The NIRC2
images yield a projected separation $\rho=0\farcs17$, with $\Delta
{\rm Kp} = 2.5$.  Using the measured Gaia EDR3 parallax for the
system, this implies a projected separation of 54\,AU.  The presence
of this star is consistent with the excess noise reported to be
present in the Gaia astrometric time-series (see
Section~\ref{app:companionstar}).  Given the low chance of a star
imaged within {\bf X.X arcseconds} to be a chance unbound companion
along this line of sight, we dub the companion star Kepler\,1627B, and
proceed under the assumption that it is bound.

Unforuntately, we do not have any reliable color information about the
secondary star.  Based on the
...
NIR-contrast measured for a main sequence G8V star corresponds to a
spectral type for the companion of $\approx$M2V ($M_\star \approx
0.44\,M_\odot$).  However, the companion should therefore have a
significantly longer pre-main-sequence contraction phase, which would
imply that this mass is an overestimate by $\approx20\%$.

Could the companion host the planet, or be creating a false positive
signal?  The star contributes $\approx$1\% of the total flux observed
in the Kepler aperture(0.7-2.0\% V-band to G-band dmag difference from
EEM table; todo is fix using model spectra).  The observed transit has
a depth of 0.18\%.  An 18\% deep eclipse of the secondary star could
produce such a signal.  Given the shape of the observed signal
(allowed impact parameters span 0.02 to 0.73, see
Table~\ref{tab:posterior}), the body transiting the secondary would
need to be non-grazing with $R/R_2 \approx 0.42$, {\it i.e.}, with
radius $\approx 0.2R_\odot$.
The duration of the transit 
($2.823 \pm 0.057 hr$) is ultimately what makes this scenario
implausible.

% {\bf TODO:
% uhhh what about the pre-MS M dwarf star being not very dense?
% What is the density of the G dwarf expected to be?
% }
% 
...
\section{Companion Star}
\label{app:companionstar}

{\bf TODO: include the RUWE vs Rp mag.}
