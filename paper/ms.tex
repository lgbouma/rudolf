...
\begin{abstract}
  ...
  using Gaia; many other new clusters overlap with confirmed and
...
\begin{figure}[t]
    ...
    or orange.  Kepler\,1627 (green star) was observed during the
    entirety of the Kepler mission.  Colors of candidate cluster
...
this measurement is also in line with expectations for a 38\,Myr star
({\it e.g.}, \citealt{randich_gaiaeso_2018};
...
\begin{figure}[tp]
    ...
    $\sim$60\,ppt per half rotation period
    ($\sim$1.3\,days).
    ...
    The best-fit sky-projected obliquity for this model is listed in
    red.  While the intra-exposure RV scatter precludes a detection of
    the RM effect, the photometric transit depths are consistent
    across the {\it griz} bandpasses.
    \label{fig:obliquity}
	}
\end{figure}
\begin{figure*}[tp]
    ...
    Planets younger than a gigayear with ${\rm \tau}/\sigma_{\rm \tau}
    > 3$ are emphasized, for $\tau$ the age.  \pn\ is shown with a
...
V1298 Tau ($23\pm4\,{\rm Myr}$; \citealt{david_four_2019}).}  A
portion of the light curve is shown with the phase-folded transit in
...
0.33\,M_\odot$).  We therefore reconsidered this false positive
...
\pm 0.0098\ {\rm ppt\,day}^{-1}$ is observed.  Less than ten Kepler
...
orbit is prograde.  The latter interpretation assumes that the
dominant photometric variability is induced by dark spots, rather than
bright faculae.  Given the observed transition of Sun-like stellar
...
show the transit, and it appears to be achromatic
(Figure~\ref{fig:obliquity}, lower panels).  Fitting the MuSCAT3
photometry with a model that lets the transit depths vary across
bandpasses, we find {\it griz} depths consistent with the Kepler depth
at 0.6, 0.3, 0.3, and 1.1-$\sigma$ respectively.  The MuSCAT3
observations also suggest a transit duration $17.4\pm3.6\,{\rm
minutes}$ shorter than the Kepler transits; further photometric
follow-up could help confirm whether such changes are indeed
happening.  For the RM analysis, the details are discussed in
Appendix~\ref{app:rm}.  While our RM data are marginally more
consistent with a prograde or polar orbit than a retrograde orbit,
given that the expected velocity anomaly ($\Delta v_{\rm RM}\approx
20\,{\rm m\,s}^{-1}$) is a factor of two smaller than the
spot-corrected exposure-to-exposure scatter, we are not in a position
to claim a spectroscopic detection of the RM effect, nor a precise
quantification of the stellar obliquity.
...
observations of spectroscopic transits could yield a more precise
stellar obliquity, and could confirm or refute the expected prograde
orbital geometry.  Similarly, transit spectroscopy aimed at detecting
outflows of hydrogen or helium could yield insight into the
evolutionary state of the atmosphere
\citep{spake_helium_2018,vissapragada_2020}.  Determining the precise
amount of high-energy irradiation incident on the planet would also
help in clarifying the expected outflow rate \citep[{\it
e.g.},][]{poppenhaeger_2021}.  Finally, a challenging but informative
quantity to measure would be the planet's mass.  This might be
possible through timing analyses of the Kepler data, or through a
high-cadence multi-color radial velocity campaign.  Measured at
sufficient precision, the mass could yield constraints on both the
planet's composition and its initial entropy
...
planet-induced transit timing or transit duration variations,
Appendix~\ref{app:flare} highlights an odd flare timing coincidence
that may be suggestive of star-planet interactions.  We encourage its
...
with precise ages below 100\,Myr include K2-33, DS~Tuc~A, HIP~67522,
...
suggestion of an aligned orbit is consistent with a quiescent
...
Spectroscopic follow-up observations of the transit would however be
worthwhile to verify whether the TTV-slope correlation is yielding the
...
The authors are grateful to T{.}~David, A{.}~Howard, J{.}~Spake, and
J{.}~Winn for illuminating discussions and suggestions, and to
...
from Table~\ref{tab:posterior}, this yields an expected 1.4\%
distortion of the $\approx$1.8\,ppt transit depth: {\it i.e.}, an
absolute deviation of $\approx$2.5\,ppm.  The observed residual has a
semi-amplitude of $\approx 50\,{\rm ppm}$.  Since the expected signal is
smaller than the observed anomaly by over an order of magnitude, gravity
...
The expectation however is for the spot-induced signals to be smooth,
...
and quadratic trend in time to fit the spot-induced signal.  We
...
the RM effect.  The reason can be understood through a
model-comparison analysis.  If we compare the model with a quadratic trend and
...
all, then the usual goodness of fit diagnostics are as follows.
% from logs/rm_freejitter.log
\begin{align}
  \chi^2_{\rm red} &= 1.45 \quad ({\rm Quadratic+RM}) \nonumber \\
  \chi^2_{\rm red} &= 1.37 \quad ({\rm Linear+RM}) \nonumber \\
  \chi^2_{\rm red} &= 1.22 \quad ({\rm Only\ Quadratic}),
\end{align}
and
\begin{align}
  {\rm BIC} &= 251.7  \quad ({\rm Quadratic+RM}) \nonumber \\
  {\rm BIC} &= 257.7  \quad ({\rm Linear+RM}) \nonumber \\
  {\rm BIC} &= 242.8  \quad ({\rm Only\ Quadratic}).
\end{align}
The ``only quadratic'' model does well in these metrics
because it can inflate the jitter term to account for any additional
scatter during the transit.  However, we cannot justify a physical
prior on the jitter term, because we do not understand the origin of
the exposure-to-exposure scatter.  As noted above, the velocity
deviations from starspots are expected to have contributions at the
stellar rotation frequency, or harmonics thereof.  This jitter is
present on the exposure timescale (15 minutes), which is only 0.4\% of
the stellar rotation period; it is not obvious that starspots could be
the culprit.

The amplitude of both the spot-induced trend and
the jitter are larger than recent comparable measurements
in systems such as AU~Mic
\citep{palle_transmission_2020}, DS~Tuc
\citep{montet_young_2020,zhou_well_2020} and TOI~942
\citep{wirth_2021_toi942}.  One possible explanation for the jitter is that it is
astrophysical in origin, and that it is caused by some novel process
operating on the surface of Kepler 1627A.  Another possibility is that our RV
analysis underestimates the measurement uncertainties; the S/N per
resolution element for each of our spectra (70 to 80) was somewhat low
due to the observing requirements, and the star is a rapid rotator.
We currently favor the latter possibility.  Observations at higher
precision remain worthwhile to clarify the orbital geometry of \pn.
...
combinations thereof.  The tidal period (half the synodic
...
insights.
Such analyses should keep in mind an important caveat:
the amplitude
distribution of M-dwarf flares extends up to many times the quiescent
flux \citep[see Figure~7 of][]{gunther_2020}.  A flare on
Kepler\,1627B producing double its quiescent white-light flux would yield
a $\approx$1\% apparent amplitude.
Such flares could represent a significant fraction
of those in the Kepler observations.
