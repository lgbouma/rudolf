...
\begin{abstract}
  ...
  by the main Kepler mission.  The Kepler photometry shows two notable
  ...
  the transit time and the local light curve slope.
  % The former could be explained by a dusty atmospheric outflow; the
  % latter could be explained by unresolved starspot crossings of a
  % planet on a prograde orbit.  
  We discuss plausible explanations for the light curve anomalies, and
  also highlight broader implications for our ability to expand the census
  of age-dated planets.
\end{abstract}
...
the discovery that Kepler observed a portion of the \cn\ (Stephenson-1; Theia~73; \citealt{stephenson_possible_1959}).
Figure~\ref{fig:skychart} shows the overlap between the Kepler field
and the cluster.  A previous clustering analysis of the Gaia
data by \citet{kounkel_untangling_2019}
reported that Kepler\,1627 (KIC 6184894; KOI 5245; TIC 120105470) is a
member of the \cn.  Given the
...
(Section~\ref{sec:planet}).  Based on a correlation between the
planetary transit times and the local light curve slope, the orbit
also seems to be prograde.  We conclude by highlighting broader
...
and accurate age for the cluster, however these are beyond our scope.
...
discussed in Appendix~\ref{app:kinematicselection} in the spatial and
...
\citep{douglas_stephanie_t_2021_5131306}.  While binarity is known to
affect the density of stars that fall beneath the slow sequence
\citep{meibom_effect_2007,gillen_ngts_2020,bouma_2021_ngc2516}, it is
not expected to change the central conclusion regarding the cluster's
age.
%%VERSION1
%Based on an analysis of photometric binaries and stars with high RUWE,
%we found evidence for similar effects to those previously noted in the
%literature.  However, since the purpose of our rotation period
%analysis is to determine the age of the \cn, we do not dwell on this
%topic.
% %VERSION0
...
consistent with a $\approx40$\,Myr age. The most precise age for \sn\ and its
...
this measurement is in agreement with expectations for a $\approx40$\,Myr star
...
\begin{figure}[tp]
	\begin{center}
		...
		\vspace{-0.1cm}
		\subfloat{
			\includegraphics[width=0.49\textwidth]{f4b.pdf}
		}
	\end{center}
	\vspace{-0.7cm}
	\caption{
...
($23\pm4\,{\rm Myr}$; \citealt{david_four_2019}).}  The light curve
and the phase-folded transit are shown in Figure~\ref{fig:lc}.  The
photometry is dominated by a quasi-periodic starspot signal with a
peak-to-peak amplitude that varies between 2\% and 8\%.  The signal
repeats every $2.642\pm0.042$\,days, where the quoted uncertainty is
based on the scatter in rotation periods measured from each individual
Kepler quarter.  Our model for the starspot and transit signals is
discussed in detail in Appendix~\ref{app:gptransit}, and the adopted
stellar parameters are listed in Table~\ref{tab:starparams}.  The
stellar mass, radius, and effective temperature are found by
interpolating against a 38$\,$Myr MIST isochrone.  Their systematic
uncertainties are taken to be the difference between the PARSEC and
MIST isochrones, and the reported uncertainties are a quadrature sum
of the statistical and systematic components.

The transit fitting results, given in Table~\ref{tab:posterior}, are
broadly consistent with a mini-Neptune sized planet ($3.4\pm
0.2\,R_\oplus$) on a close-in circular\footnote{ Our transit fitting
...
R_\odot$).  Our best-fit planet size is smaller than those
...
the different methods used to remove the stellar rotation signal.
...
0.33\,M_\odot$; $\rho\approx0\farcs16$).  We therefore reassessed
...
\pn\ is one of the smallest planets in this sample ($3.4\pm
0.2\,R_\oplus$), which could be linked to the selection effects
...
using the Gaia HR diagram and TESS stellar rotation periods to verify
...
the header of Table~\ref{tab:v06}, HD 150706b, is an example: the
host star appears to be a member of the Ursa Major moving group
($\approx 400\,{\rm Myr}$; \citealt{mann_tess_2020}).  Confirming
such kinematic associations with independent age indicators is
essential, because the false positive rates are uncharted.
...
high during the first half, and low in the second half.  The
semi-amplitude of the deviation is $\approx50\,{\rm ppm}$, which
represents a $\approx 3\%$ distortion of the transit depth
($\approx1700\,{\rm ppm}$).  To determine whether this effect was
caused by our stellar variability model, we explored an alternative
...
asymmetry was still present, at a comparable amplitude.  Note that
even though this asymmetric feature is within the 2-$\sigma$ model
uncertainties, this model itself has a jitter term that grows to
account for otherwise unmodelled white noise in the flux.  The
significance of the asymmetry is therefore best assessed in comparison
against the intrinsic out-of-transit scatter, not the model
uncertainties.
...
\begin{figure*}[tp]
    ...
    Kepler\,1627A and all members of the $\delta$\,Lyr cluster
...
existence was also suggested by the primary star's large RUWE,
...
astrometric motion.  Regardless, given the large observed RUWE of the
primary, and the low geometric probability that a companion imaged at
