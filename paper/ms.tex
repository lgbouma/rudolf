...
\begin{abstract}
  ...
  cluster.  Our analysis advances Kepler\,1627Ab as the youngest
  planet with a precise age found by the main Kepler mission.
  Analyzing the photometry, we find evidence that starspot crossings
  {\it i)} distort the average transit shape, and {\it ii)} induce
  transit timing variations that suggest the planet is on a prograde
  orbit.  The possibility that star-planet interactions could induce
  flares in the system is also explored.  Future spectroscopic studies
  of \pn\ may help clarify the early evolution of the orbit and
  atmosphere for this mini-Neptune.  More generally, the merging of
  Gaia and TESS data offers an opportunity to significantly expand the
  census of age-dated planets; we provide a literature concatenation
  of young stars to support this effort.
\end{abstract}
...
\begin{figure*}[t]
	...
	\vspace{-0.7cm}
	\caption{
    {\bf The \cn\ is $35\pm15$\,Myr old.} {\it Left:} TESS and
...
% 		is induced by starspots.  The model for the
...
\begin{figure*}[tp]
  ...
  \caption{ {\bf The light curve of Kepler\,1627.}
    {\it Top}: 
		...
		parts-per-thousand ($\times 10^{-3}$).  The dominant signal
		is induced by starspots.  The model for the
    stellar variability (orange line) is subtracted below, revealing
    the transits of \pn, as well as other deviations from the stellar
    ...
    {\it Bottom}:
		Phase-folded transit of Kepler 1627b, with stellar
			variability removed.
    The 2-$\sigma$ model uncertainties and the maximum {\it a
    posteriori} model are shown as the faint purple band, and the dark
    purple line.  Gray points are individual \texttt{PDCSAP} flux
    measurements; black points are binned to 20 minutes, with a
    representative error bar shown.
    The in-transit residual is robust against detrending methods; we
    believe it is caused by starspot crossings (see text).
    \label{fig:lc}
  }
\end{figure*}
...
known planet in the cluster: Kepler\,1627b.  Given the
planet's previous statistical validation by \citet{morton_false_2016},
we decided to examine the age of the star and its cluster more closely.
...
If the Kepler\,1627b transit signal is created by a bonafide planet,
...
the false positive scenarios of a foreground
...
smallest stars from identically-sized gas giant planets.  With a size
of $\approx 3.6\,R_\oplus$, Kepler\,1627b does not meet this
concern.
The fact that we detect a low-mass stellar companion in
...
Two additional lines of evidence ultimately persuaded us in favor of
the planetary interpretation.  First, the $\approx$100 days of
1-minute short-cadence data enables a more precise measurement of the
transit impact parameter: {\bf todo numbers} it is constrained to be
$<0.62$ (3-$\sigma$) from the 1-minute cadenced data, compared to
$<0.XX$ (3-$\sigma$) from the 30-minute data.  The second, and
ultimately convincing line of evidence came from our transit timing
analysis.  {\bf todo: KM pls check numbers} We isolated each of the
...
Figure~\ref{fig:ttvslope} against the local linear slope.  Only a
handful of Kepler Objects of Interest have shown this correlation
...
This is of course only possible if the planet is transiting the
primary star, which excludes the any of the aforementioned false
positive scenarios.
It also implies that the planet's orbit is prograde.  The latter
interpretation assumes that the dominant
...
\begin{figure*}[tp]
    ...
    The time of each transit was measured, along with the local
    slope of the light curve.  The two quantities appear anti-correlated, which is most
    easily explained by starspot crossings during the first
    (second) half of transit inducing a positive (negative) TTV,
    provided that the orbit is prograde \citep{mazeh_time_2015}.
		\label{fig:ttvslope}
	}
\end{figure*}
...
In short, Kepler\,1627Ab provides an interesting extremum in the ages
...
yield a quantification of the stellar obliquity, and a means for
verifying the suggestion of a prograde order from transit timing
(CITEP,CITEP).  Similar transit spectroscopy aimed at detecting either
hydrogen or helium absorption could yield insight into the planet's
atmosphere, which is expected to be outflowing at a rate of {\bf
...
analyses without even needing to acquire more data.  While our transit
timing study did not yield the detection of any clearly significant
planet-induced TTVs, the star's flaring may be suggestive of
star-planet interactions.  Appendix REFERENCE highlights an odd flare
timing coincidence that may be worthy of further examination.  

Ultimately though, our age measurement for \pn\ was enabled
by identifying its connection to the \cn\ through the Gaia data.  The
...
mission (CITE RickerJATIS2014).  An alternative approach also likely
to bear fruit is to identify kinematic associations around exoplanet
host stars using the Gaia data, and to then verify these associations
with ancilliary rotation periods and spectroscopy (CITEP Tofflemire).
...
K.~Masuda for advice on gravity darkening.
