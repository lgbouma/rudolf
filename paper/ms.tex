...
38\,Myr age as our reference, yielding an age for the \cn\ of
...
\begin{figure}[tp]
    ...
    {\it Bottom:} Radial velocities from
    Keck/HIRES during a transit on 7 Aug 2021 UT, observed
    simultaneously with MUSCAT3 ({\it griz} photometry).  A quadratic baseline trend was
    allowed in the RV fit.  The best-fit sky-projected obliquity in this
    model is listed in red; the detection however has marginal
    statistical significance compared to a ``trend-only'' model.
    \label{fig:obliquity}
	}
\end{figure}
...
with a peak-to-peak amplitude that varies between 2\% and 8\%,
...
the scatter observed between different Kepler quarters.
The details of how we model this starspot signal to
derive the parameters of the planet are discussed in
Appendix~\ref{app:gptransit}.  The stellar parameters are listed
in Table~\ref{tab:starparams}; the stellar mass, radius, and effective
...
($e<0.XX$), around a G8V star ($0.88 \pm 0.02 R_\odot$).  {\bf TODO
...
This implies an even stronger constraint on the transit shape in an
analysis of the false positive probability.

% TODO: Kento Masuda, please verify
...
models for the Rossiter-McLaughlin (RM) effect, and allowed three parameters
to vary: the sky-projected obliquity, the stellar
equatorial velocity, and the Gaussian dispersion of spectral lines.
We also allowed for an optional linear and quadratic trend, and
a white-noise jitter term.  The quadratic model with the RM
effect is shown in Figure~\ref{fig:obliquity}, where the 
prior on $v\sin i$ has been assumed from Table~\ref{tab:starparams}.  We found $\chi^2_{\rm
red} = 5.4$ for the best-fitting linear case, and $\chi^2_{\rm red} =
4.4$ for the best-fitting quadratic, with a $\Delta {\rm BIC} = {\rm
...
is particularly good.  Considering a quadratic trend alone {\it without any
RM effect} yields $\chi^2_{\rm red} = 4.3$, and $\Delta {\rm BIC} =
{\rm BIC}_{\rm trend} - {\rm BIC}_{\rm quad} = 1.9$ (similarly $\Delta
{\rm AIC}=5.2$).  From a model-comparison standpoint, there is therefore only
weak statistical evidence to prefer the best-fitting ``RM and
...
($\Delta v_{\rm RM}\approx 20\,{\rm m\,s}^{-1}$) and the actual achieved
precision, the claim of having detected the Doppler
transit is marginal at best.  
From the MUSCAT3 data however, we do know that the transit was observed, and the
increase in radial velocity seen shortly after ingress is more
consistent with a prograde or polar orbit than a retrograde orbit.
Fixing the orbit to be either exactly prograde or exactly retrograde,
we find $\Delta {\rm BIC} = {\rm
BIC}_{\rm retrograde} - {\rm BIC}_{\rm trend} = 18.0$; the
retrograde case is ruled out on this basis.  More precise data will
however still be needed to make a precise obliquity measurement.
...
\begin{figure*}[tp]
    ...
    having evaporated; direct measurements of the atmospheric outflows
    or planetary masses would help to confirm this picture.
    Selection effects may also be important.
    Parameters are from the NASA Exoplanet Archive (July
    2, 2021).
...
mountain.
%
This paper is based in part on observations made with the MuSCAT3
instrument, developed by the Astrobiology Center and under financial
supports by JSPS KAKENHI (JP18H05439) and JST PRESTO (JPMJPR1775), at
Faulkes Telescope North on Maui, HI, operated by the Las Cumbres
Observatory.
%
