...
\begin{abstract}
  ...
  data from Gaia with rotation periods from TESS, we
  show that Kepler\,1627 is a member of the $35\pm XX$\,Myr old
  $\delta$~Lyr cluster.  Our analysis advances Kepler\,1627Ab as the
  youngest planet with a precise age found by the main Kepler
  mission.  
  Leveraging the corresponding photometry, we explore the possibility
  that star-planet interactions induce flares in the system, and find
  evidence that starspot crossings distort the average transit shape.
  More broadly though, the merging of Gaia and TESS data offers an
  opportunity to significantly expand the census of age-dated planets;
  we provide a literature concatenation of young stars to support this
  effort.
  In the future, spectroscopic studies of \pn\ may also help clarify
  how the orbits and atmospheres of the mini-Neptune planets evolve.
\end{abstract}
...
per Kepler transit is $\approx8$, and so the individual spot-crossing
events cannot be resolved.  {\it A priori}, one would also expect the
spot-crossing events to not have a preferred phase, so that they would
average out over the $\approx200$ Kepler transits.  Nonetheless, given
the available data, it is our best explanation for what we believe is
real additional scatter during the transits, relative to the
