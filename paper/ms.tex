...
study.  Ground-based observations of its Doppler transit ($\Delta
v_{\rm RM} \approx 20$\ms) with HIRES or the upcoming KPF spectrograph
could yield a measurement of its orbital obliquity (CITEP,CITEP).
...
The precision of the Kepler data itself may enable detailed analyses
without even needing to acquire more data.
These could include studies transit timing variations, distortion of
the transit signal due to the star's gravity darkening, 
and interactions between the star and planet exhibited by patterns in
the star's flaring.
On the latter point,
Appendix REFERENCE highlights an odd coincidence: 
in the 97.7 days of available 1-minute cadence Kepler observations
(``short-cadence''), 
24 flares were observed to exceed $0.5\%$ in relative flux.
Cumulatively, these flares spanned a total of 0.27 days.
The brightest flare, highlighted in FIGURE CITE, had a flare occur
after it to within 0.2\% of an orbital period.
This seems {\it a priori} unlikely; in fact, 4 of the 24 flare events
have candidate ``successors'' (events that follow to within 2\% of the
planet's orbital period).
Further analysis would be needed to test the hypothesis of whether or
not the flare time
distribution deviated from a Poisson process...
[like: compare Poisson model versus Poisson+periodic mixture?]
