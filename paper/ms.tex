...
\begin{abstract}
  ...
  peculiarities: the average transit profile is
  ...
  More broadly applicable though is the fact that the $\delta$~Lyr
  cluster is one of $\sim$10$^3$ clusters whose properties have been
  clarified by Gaia.  Many other exoplanet hosts are candidate
  kinematic members of these clusters; their cluster memberships can
  be verified by measuring stellar rotation periods in the TESS full
  frame images.
\end{abstract}
...
data by \citet{KounkelCovey2019}
...
we begin by examining the properties of the cluster (Section~\ref{sec:cluster}).  We find that the $\delta$ Lyr
...
\begin{figure*}[t]
	...
	\caption{
		...
		Our analysis considers stars (black points)
...
\citet{KounkelCovey2019} applied an unsupervised clustering
...
cluster of identically-aged stars, they could also be heavily
...
\citet{CantatGaudin2018a}, and the field stars are from the Gaia
...
excluded stars that were obvious photometric binaries in the
...
IC\,2602 span 30\,Myr to 46\,Myr, with older ages being
...
the ``fast sequence'' of stars beneath the slow sequence
...
magnitude (mostly originating from the parallax uncertainty) and the color; the systematic
...
\begin{figure*}[tp]
    ...
    members.  Since other members of the
...
These models combined with the NIRC2 contrast limits imply
...
characterized the smaller transit signal
...
t_0, \delta, b, u_1 ,u_2 ,R_\star, \log g, e, \omega \}$),
...
stochastic noise, but found that this did not affect the
...
broadly consistent with a mini-Neptune sized planet ($3.82\pm0.16\,R_\oplus$) on a close-in circular\footnote{ Our transit fitting
...
R_\odot$).  This best-fit planet size is consistent with those
previously reported by \citet{morton_false_2016} and
\citet{berger_identifying_2018}, and corrects for the small
amount of flux dilution from Kepler\,1627B.
%at 0.3-$\sigma$ and 0.2-$\sigma$ respectively.

The transit fit however is not perfect: the lower panels of
Figure~\ref{fig:lc} show an asymmetric
residual in the data relative to the model: the measured flux is 
...
data binning in time and by local light curve slope.  The most likely
...
\begin{figure}[tp]
	...
	\caption{
		...
		\citep{mazeh_time_2015}.  The units along the abscissa can
		be understood by considering that the stellar flux changes by
...
which had a model-dependent probability of $\approx$$10^{-5}$.  However,
...
The observed transit has a depth of $\approx$0.18\%.  A
...
transit signal however requires the impact parameter to be below 0.77
(2-$\sigma$); the tertiary transiting the secondary would
...
While the planet might hypothetically still orbit a hidden close and
bright companion, such scenarios are ruled out by {\it i)} a lack of
secondary lines in the HIRES spectra and {\it ii)} a lack of
secondary rotation signals in the Kepler photometry.
...
\begin{figure}[t]
	...
	\caption{
		...
		in time to model the spot-induced $\approx$\,$250$\ms\ trend (see Appendix~\ref{app:rm}).  The radial velocity jitter
		over the 15~minute exposure timescale ($\sigma_{\rm RV}\approx30$\ms) prevented us from detecting the RM
		effect.
		The RV variations are strongly correlated with varying emission in the Ca H and K lines (middle panel).
		\label{fig:ground}
	}
\end{figure}
...
30$\ms) is comparable to the expected velocity anomaly assuming an
aligned orbit ($\Delta v_{\rm RM}\approx 20\,{\rm m\,s}^{-1}$).  We
are therefore not in a position to claim a spectroscopic detection of
the RM effect, nor to quantify the stellar obliquity.
...
given the technical difficulty of performing a multi-color radial velocity campaign
on a $V\approx13$ star.
...
asymmetry is also consistent with predictions
\citep{wang_dai_2019}.  
Beyond the asymmetric transits,
...
\citep{bouma_cluster_2020}, the two-planet AU~Mic
\citep{plavchan_planet_2020,martioli_aumicbc_2021} and the four-planet
V1298~Tau \citep{david_four_2019}.  \pn\ is one of the smaller
planets in this sample ($3.82\pm0.16\,R_\oplus$), which could be
linked to the selection effects imposed by spot-induced
photometric variability at very young ages \citep[{\it
e.g.},][]{zhou_2021_tois}.  
However,
smaller planets could have been detected:
the Kepler pipeline's median completeness extended down to
$1.6\,R_\oplus$ at 10~day orbital periods, and $3.3\,R_\oplus$ at
100~days \citep{2021ascl.soft07027B}.
The large size of \pn\ relative to the typical Kepler mini-Neptune might
therefore support a picture in which the typical mini-Neptune
($5\,M_\oplus$ to 10$\,M_\oplus$; \citealt{wu_mass_2019}) loses a
significant fraction of its primordial atmosphere over its first gigayear
...
% The alternative scenario of in-situ formation is
...
systems \citep[{\it e.g.},][]{guerrero_tess_2021}. Confirming
these candidate associations using independent age indicators is
essential because their false positive rates are not known.
A related path is to identify new kinematic
...
Each approach seems likely to expand the census of planets with precisely
measured ages over the coming years, which will help in
deciphering the early stages of exoplanet evolution.
...
R{.}~Kerr for providing us with the \citet{Kerr2021} membership
...
prefer stars with known ages, we made a few additional
...
available, we either followed the spatial (plus
...
%     \citet{KounkelCovey2019}.  We focus on stars (black points)
%     in the spatial and kinematic vicinity of Kepler\,1627 (yellow
...
% \citet{KounkelCovey2019} to be in the group ``Theia 73'', which
% was cross-matched by Kounkel \& Covey as being ``Stephenson 1''.
% \citet{KounkelCovey2019} reported \noriginal\ stars to be
...
% \citet{KounkelCovey2019}.  We therefore opted to only consider
...
\begin{figure}[tp]
	...
	\vspace{-0.7cm}
	\caption{
    {\bf Transit models and residuals, binned by quartiles in the local slope of the
    light curve}.  
...
and~\ref{fig:phasettv} show the Kepler data binned in four ways:
over Kepler quarters, Julian years, quartiles of local slope, and quartiles of
transit timing variation.  Over Kepler quarters
(Figure~\ref{fig:phasequarter}), Quarter 6 shows the strongest asymmetry: a deviation of about 3\,ppt from expectation.
...
visually present in three of the four quartiles: the only bin in
which it does not appear is ${\rm d}f/{\rm
d}t\in[0.001,0.033]\,{\rm ppt\,day}^{-1}$.  The corresponding bins in TTV
(largest TTV corresponding to smallest ${\rm d}f/{\rm d}t$)
show the same asymmetries, though with somewhat greater scatter
...
expect the light curves with the most negative local slopes
to have the largest positive TTVs, due to spot crossing events during the latter
half of transit.  The lower-right panel of Figure~\ref{fig:phasettv}
and upper-left panel of Figure~\ref{fig:phaseslope} agree with this
expectation.  However, we would also expect the sign of the effect to
reverse when considering the most positive local slopes, and the most
negative TTVs.  The upper-left panel of Figure~\ref{fig:phasettv} and
lower-right panel of Figure~\ref{fig:phaseslope} contradict this
expectation: the residual in both cases maintains the same
parity!  On the one hand, this shows us that the residual is not
dependent on the local light curve slope, which mitigates the
possibility that it might be an artifact of our detrending methods.
On the other, it raises the question of whether
unresolved starspot crossings are indeed the root cause of the correlation shown in
Figure~\ref{fig:corr}.
...
Since the transit asymmetry seems robust against most methods of
binning the data, we considered four possible astrophysical
interpretations: gravity darkening, transit timing variations,
spot-crossing events, and a persistent asymmetric dusty outflow.  
...
The airmass ranged between 1.1 and 2.2 from the
...
$\approx$250\ms\ over the six hour window.  
This decrease in RV is correlated with a decrease in the S-indices
derived from the Ca HK lines.
One outlying RV point is apparent shortly before egress; it is
temporally coincident with an outlying value in the S-index time series.
...
the overall $\approx$250\ms\ trend.  The Ca HK and H$\alpha$ emission
...
and quadratic trend in time to fit the $\approx$250\ms\ spot-induced trend.  We
...
\citet{claret_gravity_2011}.  We assumed a Gaussian prior on $v\sin i$ and
...
uncertainties.
We used a 15\,minute exposure time to numerically evaluate the
model.
...
on the RVs ($\approx 13\,{\rm m\,s}^{-1}$), and are dominated by the
...
particular model is $\sigma_{\rm RV} = 27^{+6}_{-5}$\ms, which is
comparable to the expected RM anomaly of $\Delta v_{\rm RM}
\approx f_{\rm LD} \cdot \delta \cdot v\sin i \cdot \sqrt{1-b^2}
\approx 20\,{\rm m\,s}^{-1}$, assuming a perfectly aligned orbit.
...
e.g.},][]{gibson_2016_kpf,seifahrt_maroon-x_2018}, or in the near-infrared
\citep[{\it e.g.},][]{feinstein_halpha_2021}.
