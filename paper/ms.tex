...
\begin{figure}[tp]
    ...
    could be explained by starspot crossings during the first
...
for \sn\ (TTVs or TDVs).  Quantitatively, the mean and standard deviation of the
...
between the two quantities.  While this constitutes only weak
statistical evidence, given the possible connection between such
correlations and the unresolved starspot crossings that we expect to
be present in the data for \sn\ \citep{mazeh_time_2015}, we opted to re-examine the
...
similar anti-correlations in the injected transits to that present in the real
transit;
...
transits.
Possible interpretations are discussed below, however given the
...
lines in the HIRES spectra and {\it ii)} the lack of secondary
rotation signals in the Kepler photometry.
...
TTVs and the local light curve slope could be a further line of evidence
in support of the planetary interpretation.  Unless it is a
statistical fluke (a $\approx$5\% possibility), then its most likely
cause is unresolved starspot
crossings \citep{mazeh_time_2015}.  This would only be possible if the
planet transits the primary star, which excludes a background
eclipsing binary scenario.  It would also suggest that the planet's
...
Kepler ephemeris and transit depth, the MuSCAT3 observations also
suggested a transit duration $17.3\pm4.3\ {\rm minutes}$ shorter than the
Kepler transits.
However, given both the lack of transit duration variations in the Kepler data
...
This somewhat contrived geometry would be necessary in order to
explain why the starspot crossings would add up coherently.
...
Aside from the asymmetric transits, Appendix~\ref{app:flare}
...
$0.66\pm5.53$ minutes; similarly the mean local slope and its
