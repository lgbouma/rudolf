...
which the planetary evidence comes from transits, and the age 
...
\citep{meingast_psceri_2019,Meingast2021,gagne_number_2021}.  
Even though these streams can be spread out over tens to hundreds of parsecs,
their velocity dispersions can remain coherent at the $\sim$1\kms\ level.
They can also be much larger:
as an extreme example, internal dynamics and projection effects
in the Hyades drive stars in the tidal tails span up to $\pm 40\mkms$
in relative velocity when projected against the cluster center \citep{jerabkova_800_2021}.  
The stars
...
magnitude diagrams \citep{KounkelCovey2019}, stellar rotation periods
...
system has spectral type $\approx$K8V.  And since a
...
Neither of the KOI-7913 components shows strong lithium absorption,
but this is expected given their $\approx$K6V and $\approx$K8V
spectral types.
They do however both show H$\alpha$ in emission.
We discuss the significant in greater depth in Appendix~\ref{app:spectra},
but broadly speaking both observations are consistent with
a $\approx$40 Myr age for KOI-7913.
...
\paragraph{Kepler-1643}
  %Based on the Q1-Q17 DR25 False Positive Probability table hosted at
...
The reports show however that two outlying quarters (2 and 6) 
drive the offset -- the centroid locations from the other Kepler
quarters are consistent at $\lesssim 0\farcs4$.  Nonetheless, this
is an instructive exercise in how stellar variability complicates
...
centroids.  For stars with significant spot-induced
variability there is no static baseline in either the in- or
out-of-transit phases.
...
routine failed, presumably due to the spot-induced variability.
%The Q1-Q17 DR25 False Positive Probability table however lists its
%probability of being an astrophysical false positive as ``1.0''. Given
%that none of the other columns in the table are populated, this seems
%likely to be a database entry error ({\bf ask Jessie}). 
Our fitting approach rectifies this point, and our new NIRC2 images
...
The positional probability score also meets the requirements
for transiting sources thought to share positions with their
target stars \citep{morton_false_2016,2017ksci.rept...16B}. 
%A visual inspection of
%each individual transit also shows a weak notch at the predicted
%transit times; over the 198 observed transits, the resulting signal
%seems sensible.
...
statistically significant ($0\farcs2$; 3.0-$\sigma$).  Again,
the data validation reports show that the shift is caused by a few
outlying quarters (4, 5, 8, and 12).  The fact that the remaining data
show a consistent scatter between centroid locations suggests that
these outliers are also caused by the stellar variability.  Independently, our
imaging shows that there are no neighboring sources that could
cause an offset of the observed amplitude.


\paragraph{KOI-7913}
KOI-7913.01 is also listed on the NASA Exoplanet Archive as a
``candidate'' planet.  The canonical \citet{morton_false_2016} analysis
was of Q1-Q17 KOIs from DR24, and therefore spanned KOI-1.01 to
KOI-7620.01 and omitted KOI-7913.01.  However the results of the
...
KOI-7913~B, which is located approximately one Kepler pixel away from Kepler-7913 A (see
Appendix~\ref{app:koi7913}).  Our pixel-level analysis of the target
pixel files did not show any evidence to support the idea that
KOI-7913~B could host the transiting planet.  Our NIRC2 imaging
similarly ruled out the possibility of any other plausible host star
for the planet other than KOI-7913~A.
