...
% 208 words (250 max)
\begin{abstract}
  Stellar positions and velocities from Gaia are yielding a refined
  view of open cluster dispersal.  Here we present an analysis of a
...
analyses have mostly clustered on the stellar positions and 2-D
velocities.  One important result is the discovery of
...
$\sim$1\kms\ level.  They can also be much larger: as an extreme
example, internal dynamics and projection effects in the Hyades drive
stars in the tidal tails to $\pm 40\mkms$ in relative velocity
when projected against the cluster center \citep{jerabkova_800_2021}.
The stars in such diffuse regions can be verified to be the same age
as the core members ({\it i.e.}, coeval) through analyses of
...
Kepler-1627 in order to secure the planet's age.  However it became
...
here on the latter three.  After analyzing the relevant properties of
...
directions of future work (Section~\ref{sec:disc_conc}).
...
\mkms,-4 \mkms)$.  RSG-5 is visible at $(l,b)=(83^\circ,6^\circ)$,
$(v_{l'}, v_b)=(5.5 \mkms,-3.5 \mkms)$.  Most of the other
...
rate determined from the CAMD seems consistent with our fraction of
detected rotation periods, given that RSG-5 was also missing rotation
period detections for $\approx$15\% of its candidate members, which
all seemed photometrically consistent with being part of a single
...
consistent with the estimated false positive rates determined from the
...
Some salient stellar properties of the Kepler objects in Cep-Her can
be gleaned from Figure~\ref{fig:age}.  They span spectral types of G8V
(Kepler-1627) to K6V (KOI-7913 A).  The secondary in the KOI-7913
system has spectral type $\approx$K8V.  And since a Solar-mass star
with solar metallicity arrives at the zero-age main sequence at
$t\approx40$ Myr \citep{choi_mesa_2016}, these stars are all in the
late stages of their pre-main-sequence contraction.  Their
heliocentric distances are $\approx$260\,pc (KOI-7913 and KOI-7368, in
CH-2) and $\approx$340\,pc (Kepler-1643, in RSG-5).
...
the two sequence epochs was $-9.1 \pm 1.9\,\mkms$ and $-7.8\pm
...
stellar parameter classification pipeline for the TRES spectra has
been described by \citet{2021tsc2.confE.124B}.  It is based on the
synthetic template library originally constructed by
...
reduction was identical as for Kepler-1643.  No companions were
...
They do however both show H$\alpha$ in emission.  We discuss the
significance in greater depth in Appendix~\ref{app:spectra}, but
broadly speaking both observations are consistent with a $\approx$40
Myr age for KOI-7913.
...
that were observed by Kepler: namely RSG-5, CH-2, and the $\delta$~Lyr
cluster.
The membership and kinematics of the other components shown 
in Figure~\ref{fig:XYZvtang} also deserve attention.
R.~Kerr and collaborators have acquired
{\bf N spectra} from which radial velocities will be derived in their upcoming study, but if the estimate that
Cep-Her has $\approx$3{,}500 stars is correct then far more might be required
to fully untangle the kinematics of the various groups.
Wide-field spectroscopic surveys such as LAMOST ({\bf cite}) or
APOGEE ({\bf cite}) could provide sensitivity to the Li 6708\,\AA\ line,
which would enable a more detailed analysis of its abundance across
FGK stars.
Gaia DR3 may also provide RVS spectra
for some of the brighter F and G dwarfs, which could provide the calcium
infrared triplet as another age indicator.

A number of worthy photometric projects also seem possible
given the new understanding of the complex.
One is asteroseismology of the $\delta$~Sct stars,
using either TESS or Kepler data.
For cases in which the modes are resolved, this might yield
age estimates for subgroups independent from other
approaches.
Other projects could include searches of the Kepler data
for exocomets (CITE Zieba), or for missed planets around the
most rapid rotators.


\paragraph{Exoplanet demographics at early times}
Our main motivation for finding new young planets is to help
benchmark models for planetary evolution.
However demographic analyses of the known planets between $10^7$ and $10^9$ years
(Figure~\ref{fig:age}) have so far been somewhat limited.
Approximately 40 such planets
are now known.
The majority of published cases ($\approx$20) come from
K2, though now $\approx$10 come from Kepler, and $\approx$10 
from TESS.

Given these detections, 
a few reflections regarding the experimental design of any
demographic survey might be helpful.
The first is that the list of target stars is an incredibly important
ingredient, and the best way to compile relevant stars right
now is likely through a combination of automated spatio-kinematic clustering
from Gaia and rotation-period measurement with TESS.
The second is that the planets $<$$3$\,$R_\oplus$ in size
all come from either K2 or Kepler.
So demographic inferences based on TESS
for the time being are limited to 
$4$ to $10$\,$R_\oplus$, close-in to the host stars.  
It seems worthwhile to comparable
the absolute occurrence rate of such planets to those derived from
Kepler (CITE Petigura).
One question that seems like it should be within reach would be to clarify
whether enough young stars have been searched for the dearth of
young hot Jupiters shown in Figure~\ref{fig:age} to be significant.
Since the hot Jupiter occurrence rate is strongly dependent on stellar
mass and metallicity (CITE), particular care would be needed to select
a sample of FGK dwarfs for the measurement, likely going out
to distances of $\approx$500\,pc to get enough stars.
The major sources of stars in the youngest age bins would
need to include Sco OB2, Cep-Her, and Orion, among others.
For studies of how mini-Neptune sizes evolve, the combined K2 and Kepler
dataset might be the better primary source.
Though the selection function is more stringent, for a given star
Kepler acquired $\sim$100$\times$ more photons.  Given the
star-count limited nature of a cluster-based exoplanet demographic
survey, every star counts.
...
are 40 to 50 million years old, and that each signal
...
For both cases, we have argued the weak observed shifts seem to be
caused by starspot-induced variability in specific quarters
shifting the stellar center-of-light.
In addition, our imaging rules out companions stars with the necessary
brightness and position to explain the reported shifts.
...
rotation period period agrees with this assessment, its lithium
content is somewhat low, which might motivate future exploration
...
The astrophysical significance of these somewhat detailed considerations
...
presented are well above the average detection significance of Kepler 
planets.  There may well still be room at the bottom.
...
\begin{figure*}[tp]
    ...
    objects of interest compared to young open clusters and
    field stars. }
	Positive equivalent width means absorption; negative
...
%$\approx$K6V, while the secondary is $\approx$K8V.  At $\approx$40
...
The most obvious explanation for KOI-7913 is the latter case, given
that KOI-7913 B is $\approx0.9$ Kepler pixels away from Kepler-7913 A
and so it usually part of the ``halo''.  Due to the on-sky orientation
of KOI-7913 A and KOI-7913 B, the default ``optimal aperture''
selected in quarters 3, 7, 11, and 15 in fact included both stars,
while for the remaining quarters KOI-7913 B was excluded from the
optimal aperture but was included as part of the halo (see pages 35
through 71 of the data validation reports.)  
...
\input{source_Kepler_1643_simpletransit_posteriortable.tex}
\input{source_KOI_7368_simpletransit_posteriortable.tex}
\input{source_KOI_7913_simpletransit_posteriortable.tex}
