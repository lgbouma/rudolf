...
\begin{abstract}
  ...
  The latter Neptune-sized planet is in a component of the Cep-Her
  ...
  and spectroscopy, these three systems are also $\approx$40 million
  ...
  mini-Neptunes with sizes of $\approx$2 Earth radii exist at ages of
  $\approx$40 million years.
\end{abstract}
...
velocities measured by Gaia.  One important result is the discovery of
...
though these streams can be spread out over tens to hundreds of
...
stars in the tidal tails span up to $\pm 40\mkms$ in relative velocity
...
resulting flattened transits were then fitted with a planetary transit
under the assumption of quadratic limb darkening.  Our model therefore
included 8 free parameters for the transit ($\{P, t_0, \log R_{\rm
p}/R_\star, b, u_1 ,u_2, R_\star, \log g\}$), 2 free parameters for
the light curve normalization and a white noise jitter ($\{\langle f
\rangle, \sigma_f \}$), and an additional 5 implicit parameters per
transit through the polynomial.
...
incorporate variability that was not part of the transit, even if it
was short-term variability attributable to flares, or simply
...
parameters about which we care the most.
...
high-resolution spectrograph.  In
...
6\times10^{-9}$, where the improvement is due to the new NIRC2
contrast curves.  Figure~\ref{fig:planets} shows the justification:
the transit is flat and has a high S/N ($\approx 47$).  The shape is
therefore difficult to reproduce with eclipsing binary models.

Intriguingly, Kepler-1643 formally failed one of the centroid shift
tests (see the \texttt{q1\_q17\_dr25\_koi} data release): the angular
distance between the target star's catalog position and the position
of the transiting source was measured as $1\farcs0$ at 4.4-$\sigma$.
The reports show however that two outlying quarters (2 and 6) drive
the offset -- the centroid locations from the other Kepler quarters
are consistent at $\lesssim 0\farcs4$.  Nonetheless, this is an
instructive exercise in how stellar variability complicates
centroid-based vetting tests.  The centroid shifts measured by these
tests are determined from the in- and out-of-transit flux-weighted
centroids.  For stars with significant spot-induced variability there
is no static baseline in either the in- or out-of-transit phases.
...
distribution peaked at $\approx$$4$\,M$_\oplus$, an ice-poor rock/iron
...
for the origin of the radius valley.  If the radius gap is primordial
...
most stars do not have dozens of $\sim$40\,Myr neighbors within a
...
\paragraph{More objects}

* TESS data: giant planets in Cep-Her.  We need more 1e7-1e8 year old
stars for the HJ occurrence versus time question.  This will help.

* Kepler data: exocomets?  Search for smaller planets?

\subsubsection{Occ rates}
...
We have argued that Kepler-1643~b, KOI-7368~b, and KOI-7913~Ab {\it i)}
