...
Table~\ref{tab:cepherkepler} contains candidate Cep-Her members with
weights $\alpha>0.02$ observed by Kepler.  A cursory inspection of the
color absolute-magnitude diagram suggests that more restrictive weight
cuts would be needed to remove the majority of field star interlopers.
Table~\ref{tab:cepherkepler}  was created by cross-matching candidate
Cep-Her members (selected using Gaia EDR3;
...
Bedell).  The \texttt{kic_\dr2\_ang\_dist} column is from the latter
table.  The EDR3 to DR2 match was performed using the
\texttt{gaiaedr3.dr2_neighbourhood} table, and the closest (proper
motion and epoch-corrected) angular distance neighbor was taken as the
as single single match.  The \texttt{edr3\_dr2\_mag\_diff} column
gives some indication of the reliability of this EDR3 to DR2
conversion, as there are likely a few cases where partially resolved
binaries became fully resolved.
