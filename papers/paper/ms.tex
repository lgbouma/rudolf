...
\replaced{To date these analyses have mostly clustered on stellar
positions and 2D velocities}{So far, these analyses have mostly
...
2022)\added{. Important caveats, particularly for extended groups $\gtrsim 100$\,Myr
...
sub-populations\added{ of Cep-Her}, dubbed ``Cepheus-Cygnus'', ``Lyra'', and
...
these groups in \replaced{space-velocity}{both space and velocity} coordinates.  In other words, the stars
...
metric. 
\added{Core distance is the distance to the $k^{\rm th}$ nearest star,
...
These groups are then used as the ``seed'' populations\replaced{ for
the third step, which is to search for objects at least as close to
the 10$^{\rm th}$ nearest HDBSCAN-identified member in space-velocity
coordinates}{, in which the stellar members each have their own
...
distance has $D=0$, and \replaced{weights for the other group members are
log-normally distributed between these two extremes}{the weight $D$
...
this procedure yields \replaced{$D \sim \log_{10}\mathcal{N}(-1.55,0.61)$}{a distribution of weights $D$
...
4{,}763 objects with $D>0.10$ as black points.  \added{These thresholds were
...
amplitude \added{for stars at least as young as the Pleiades} \citep[{e.g.},][]{rebull_rotation_2020}. 
...
members.  These stars are listed in Appendix~\ref{app:members}, \replaced{as are
the Cep-Her candidates that were observed by Kepler}{as is the set of Cep-Her
...
\begin{figure*}[tp]
    ...
    to emphasize the pre-main-sequence\added{; approximate spectral types are shown on the upper axes}.  Stars that fall far below the
...
color \replaced{range}{axis} to best visualize the region of maximal age information
...
overlap with the $\delta$~Lyr cluster\added{ on the CAMD}, and that the groups are
...
($\sim$$8$\,Myr), and some \deleted{may} still exist at the age of
...
\added{in reddening-corrected absolute $G$-band magnitude as a function of $\bpmrpo$\ color}
...
To verify these parameters\added{, determine the stellar effective temperatures,} and to analyze youth
proxies such as the Li 6708\,\AA\ doublet and H$\alpha$, we acquired
\added{high resolution optical } spectra.  We also acquired high resolution imaging for each system, to
...
respectively\replaced{ \citep{chubak_2012}}{, and was calculated
...
described by \citet{bouma_2021_ngc2516}. \added{In brief, this
...
sequence in the \replaced{left-panel}{top panel} of Figure~\ref{fig:lithium}, though
...
The \replaced{right}{middle} panel shows \replaced{the}{a} comparison against the Pleiades, where
...
stars may not fully explore all possible \deleted{stellar rotation
...
there should be a \added{single} local maximum in planet occurrence rates at 2 to 3\,$R_\oplus$,
at times between 10 and 100\,Myr. \added{In other words,
...
agree\added{ that this local maximum should exist};
...
both show weak centroid offsets \replaced{between the in
and out-of-transit phases}{in specific quarters}.  For both \replaced{cases}{systems}, the observed shifts
