...
def calculate_XYZ_given_LBPLX(l, b, plx):
    """
    Given numpy arrays of galactic longitude, latitude, and parallax, calculate
    the corresponding galactic XYZ coordinates.  NOTE: this code converts from
    parallax to distance assuming parallax [arcsec] = 1/distance[pc].  This
    assumption is wrong in the regime of low signal-to-noise parallaxes (very
    faint stars).
    ...
    Args:
        ra/dec/plx: np.ndarray of galactic longitude, latitude, and parallax.
        l/b in units of degrees.  Parallax in units of milliarcseconds.
    ...
    _c = coord.SkyCoord(
        l=nparr(l)*u.deg, b=nparr(b)*u.deg, frame='galactic'
    )

    ra = nparr(_c.icrs.ra.value)
    dec = nparr(_c.icrs.dec.value)

    x,y,z = calculate_XYZ_given_RADECPLX(ra, dec, plx)
